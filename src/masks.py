...
def get_mask_card_number(card_number: int) -> str:
    ...
    mainLog().debug("Запуск Функция маскировки номера банковской карты")
...
def get_mask_account(acc_number: int) -> str:
    ...
    mainLog().debug("Запуск Функцию маскировки номера банковского счета")
    s_acc_num = str(acc_number)
    if len(s_acc_num) != 20:
        print("Длиннна номера счета должна быть 20 символов!")
        mainLog().error("Длиннна номера счета должна быть 20 символов!")
        return ""
